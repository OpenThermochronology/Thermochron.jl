    ...
    function MCMC(chrons::AbstractVector{<:Chronometer{T}}, damodels::AbstractVector{<:Model{T}}, params::NamedTuple, boundary::Boundary{T}, constraint::Constraint{T}=Constraint(T), detail::DetailInterval{T}=DetailInterval(T); 
        ...
        rescalesdd = (haskey(params, :rescalesdd) ? params.rescalesdd : false)::Bool
        rescalemdd = (haskey(params, :rescalemdd) ? params.rescalemdd : true)::Bool
        ...
        (any(x->isa(x, VitriniteRo), chrons)) && @info "Inverting for vitrinite reflectance of $(count(x->isa(x, VitriniteRo), chrons)) samples"
