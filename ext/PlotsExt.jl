module PlotsExt
    ...
    function Plots.plot(d::ZRDAAM, dms::Vector{<:Thermochron.Model}; framestyle=:box, layout=(2,1), size=(600,800), alpha=0.75, kwargs...) 
        ...
        histogram!(he, Ea; normalized=true, lw=0, color=lines[1], label="", bins=(minimum(Ea)-0.01):0.01:(maximum(Ea)+0.01), alpha, kwargs...)
    ...
    function Plots.plot(d::RDAAM, dms::Vector{<:Thermochron.Model}; framestyle=:box, layout=(2,1), size=(600,800), alpha=0.75, kwargs...) 
        ...
        histogram!(he, Ea; normalized=true, lw=0, color=lines[1], label="", bins=(minimum(Ea)-0.02):0.02:(maximum(Ea)+0.02), alpha, kwargs...)
        Ea₀ = Normal(log10(d.EaL), d.EaL_logsigma/log(10))
        x = range(mean(Ea₀)-3std(Ea₀), mean(Ea₀)+3std(Ea₀), length=100)
        plot!(he, x, pdf.(Ea₀,x), color=lines[1], label="EaL", kwargs...)
        Ea = dms .|> x->log10(x.EaTrap)
        histogram!(he, Ea; normalized=true, lw=0, color=lines[2], label="", bins=(minimum(Ea)-0.02):0.02:(maximum(Ea)+0.02), alpha, kwargs...)
    ...
    function Plots.plot(d::MDDiffusivity, dms::Vector{<:Thermochron.Model}, r=100.0; framestyle=:box, layout=(2,1), size=(600,800), alpha=0.75, kwargs...) 
        ...
        for j in 1:ndomains
            ...
            histogram!(he, Ea; normalized=true, lw=0, color=lines[j], label="", bins=(minimum(Ea)-0.0025):0.005:(maximum(Ea)+0.005), alpha, kwargs...)
